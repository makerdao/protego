...
function analyzeEvents(events) {
    ...
    return {
        ...
        pendingCount:
            plotEvents.length - (execEvents.length + dropEvents.length),
        events: {
            plot: plotEvents,
            exec: execEvents,
            drop: dropEvents,
...
describe("fetchPausePlans", () => {
    ...
    it("should fetch spells from specified block number", async () => {
        ...
        expectedEvents.forEach((event) => {
            ...
            const plotEvent = spellEvents.find(
                (e) => e.topics[0] === PLOT_TOPIC,
            );
            const execEvent = spellEvents.find(
                (e) => e.topics[0] === EXEC_TOPIC,
            );
            const dropEvent = spellEvents.find(
                (e) => e.topics[0] === DROP_TOPIC,
            );
